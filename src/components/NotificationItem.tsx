      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-preset-4-medium md:text-preset-3-medium text-gray-600 leading-snug">
            <span className="text-preset-4-bold md:text-preset-3-bold text-navy-950 inline mr-100">{sender.name}</span>
            {' '}
            {message}
            {linkText && (
              <>
                {' '}
                <span className="text-preset-4-bold md:text-preset-3-bold text-gray-600 inline ml-100">{linkText}</span>
              </>
            )}
            {groupName && (
              <>
                {' '}
                <span className="text-preset-4-bold md:text-preset-3-bold text-blue-950 inline ml-100">{groupName}</span>
              </>
            )}
            {/* Unread Dot / 안읽음 표시 */}
            {!isRead && (
          ...
          {type === 'message' && privateMessage && (
            <div className="border border-navy-100 rounded-[5px] p-200 md:px-300 mt-100 text-preset-4-medium md:text-preset-3-medium text-gray-600">
              {privateMessage}
            </div>
          )}
        </div>
